...
def DiagonalLSTM(name, input_dim, inputs):
    ...
    #)
    c0_unbatched = numpy.zeros((HEIGHT, DIM), dtype=theano.config.floatX)
    ...
    #    numpy.zeros((HEIGHT, DIM), dtype=theano.config.floatX)
    #)
    h0_unbatched = numpy.zeros((HEIGHT, DIM), dtype=theano.config.floatX)
...
grads_ = T.grad(cost, wrt=params, disconnected_inputs='warn')
grads = [T.clip(g, lib.floatX(-GRAD_CLIP), lib.floatX(GRAD_CLIP)) for g in grads_]
...
def generate_and_save_samples(tag):
    ...
    def save_images(images, filename):
        ...
        if not os.path.exists('./out'):
            os.makedirs('./out')
        scipy.misc.toimage(images, cmin=0.0, cmax=1.0).save('./out/{}_{}.jpg'.format(filename, tag))
...
print "Training!"
helper.check_and_create_dir('./out')
total_iters = 0
total_time = 0.
last_print_time = 0.
last_print_iters = 0
for epoch in itertools.count():
    ...
    for images, targets in data_feeder:
        ...
        cost, outputs = train_fn(images)
        total_time += time.time() - start_time
        total_iters += 1

        '''
        print 'images:', outputs[0]
        skewed = skew_fn(outputs)
        print 'skewed:', skewed[0]
        '''
        ...
        if (TRAIN_MODE=='iters' and total_iters-last_print_iters == PRINT_ITERS) or \
            ...
            #helper.save_images(images, 28,28,10,10,dir='./out')
